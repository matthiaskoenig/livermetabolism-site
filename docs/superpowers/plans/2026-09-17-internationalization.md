...
Pages to convert: all 13, including the `quickLinks` and per-tag link-card arrays in `index.astro` and the `positions` array in `people.astro`. The `people.astro` positions text contains inline HTML rendered with `set:html`; keep it as HTML in the catalog value and keep the `set:html` render, since it is the site's own copy, not user input.
