...
describe('homeFigures', () => {
  it('lists the six figures in the strip order', () => {
    expect(homeFigures(input).map((f) => f.id)).toEqual(['publications', 'citations', 'h-index', 'team', 'software', 'funding']);
  });

  it('takes every number from the data and the two snapshots', () => {
    const by = Object.fromEntries(homeFigures(input).map((f) => [f.id, f]));
  ...
  it('labels the figures and links each one to its page', () => {
    ...
    expect(homeFigures(input).map((f) => [f.label, f.href])).toEqual([
  ...
  it('omits citations and the h-index when the Scholar snapshot is empty', () => {
    const figures = homeFigures({ ...input, scholar: { fetchedAt: epoch, citations: { all: 0 }, hIndex: { all: 0 } } });
  ...
  it('omits the stars line (but not the figure) when the GitHub snapshot is empty', () => {
    const figures = homeFigures({ ...input, github: { fetchedAt: epoch, repos: {} } });
  ...
  it('sums the stars over every repository of the snapshot, and writes "1 star" singular', () => {
    const one = homeFigures({ ...input, github: { ...github, repos: { 'a/one': { stars: 1 } } } });
    expect(one.find((f) => f.id === 'software')!.sub).toBe('1 star');
    const none = homeFigures({ ...input, github: { ...github, repos: {} } });
  ...
  it('omits no figure for empty data (a real 0 is a number, not a missing snapshot)', () => {
    const figures = homeFigures({ ...input, publications: [], people: [], software: [], funding: [] });
  ...
  it('gives every figure a plain integer value', () => {
    for (const f of homeFigures(input)) expect(String(f.value)).toMatch(/^\d+$/);
  });
});
