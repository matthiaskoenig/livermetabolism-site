...
<template>
  <div class="tag-filter" :id="`${id}-tag-filter`">
    <button type="button" class="tag-filter-btn" :class="{ active: modelValue === 'all' }" :aria-pressed="modelValue === 'all'" data-tag="all" @click="emit('update:modelValue', 'all')">All</button>
