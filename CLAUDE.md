...
- `site/lib/githubRows.ts` holds every projection of the snapshot (`statsFor`, `latestReleases`, `releaseTimelineRows`, `starsRows`, `activityRows`, `relativeDate`, `shortDate`) as pure functions, so the build and the browser compute the same rows; `site/lib/chartOptions.ts` turns those rows into ECharts options. On `research.astro` the rows are props of four `client:visible` islands (`ReleaseFeed.vue`, `ReleaseTimeline.vue`, `CommitActivityChart.vue`, `StarsChart.vue`, the last three through `useChart.ts`), each of which re-computes its rows when `loadLiveSnapshot()` returns something newer than the build's `fetchedAt`; the per-card `.software-stats` lines (`SoftwareCard.vue`, `[data-repo]` with `[data-field]` spans) are patched in place by `site/lib/githubStats.ts` from the bundled script of `SoftwareLive.astro`, which also renders the "Data from GitHub, updated …" note.
