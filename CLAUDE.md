...
- **Site chrome**: the navbar toggle (`TopNav.astro`), the cookie banner (`CookieConsent.astro`), the search dialog (`SiteSearch.astro`), and the modal router (`Base.astro`) are on every page, so none of them is a Vue island: each is static markup plus a bundled Astro `<script>` block (TypeScript, hashed for the CSP, deduplicated per page, runs once after parse) that imports the shared logic from `site/lib/*.ts`. Configuration reaches those scripts through `data-*` attributes on the component's root element (`data-ga-id`, `data-search-url`), never through props. Keep new chrome that way; `client:` directives are for the two islands above.
