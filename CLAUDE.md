...
- ECharts is imported from `echarts/core` with only the used pieces (bar + scatter series, grid/tooltip/legend/dataZoom, `CanvasRenderer`) — still a ~180 KB gzipped chunk, which is why it loads only on the research page and only once a chart scrolls into view. Tooltips use `renderMode: 'richText'` (drawn in the canvas): an HTML tooltip would inject inline styles and trip the CSP. Chart heights are set through the CSSOM in `useChart.ts` for the same reason.
...
- A blocked request (non-200, a redirect to `/sorry/`, the CAPTCHA form) throws with a message starting with `blocked` and **writes nothing**; the workflow step is `continue-on-error: true` and runs after the worktree is prepared, so a blocked run neither fails the GitHub snapshot nor replaces the last good `scholar.json`.
...
- **Static-first islands**: list pages are plain HTML. One Vue SFC per entry type (`PublicationRow`, `ProjectCard`, `PersonCard`, `NewsCard`, `SoftwareCard`, `FundingCard`, `EditorCard`, `MeetingCard`, `PresentationCard`, `PosterCard`, `AbstractCard`, `TeachingCard`, …) plus every modal (`PersonModal`, `ProjectModal`, `NewsModal`) is rendered from the `.astro` page **without** a `client:` directive, so no record is ever serialised a second time into `astro-island` props. Keep it that way: a new card or modal belongs in the page's `.map()`, not in a wrapper island. Only two components hydrate, and only these may take a `client:` directive:
  ...
  - `ReleaseFeed.vue`, `ReleaseTimeline.vue`, `CommitActivityChart.vue`, `StarsChart.vue` (`client:visible`, research page only) — the live GitHub sections: the feed is server-rendered and hydrates only to refresh itself, the three charts need a canvas (see "Live GitHub data").
