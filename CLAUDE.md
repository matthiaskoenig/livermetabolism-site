...
- **Components**: one Vue SFC per entry type (`PublicationRow`, `ProjectCard`, `PersonCard`, `NewsCard`, `SoftwareCard`, `FundingCard`, `EditorCard`, `MeetingCard`, `PresentationCard`, `PosterCard`, `AbstractCard`, `TeachingCard`, …), used statically (no `client:` directive) from `.astro` pages wherever no interactivity is needed. Section islands (`*Section.vue`: `ProjectsSection`, `PublicationsSection`, `NewsSection`, `SoftwareSection`, `FundingSection`, `EditorsSection`, `MeetingsSection`) and `PersonAvatar.vue` hydrate `client:visible`, deferring their JS until the section scrolls into view; `SiteSearch.vue` stays `client:idle` (it must be ready for the `/`/Cmd-K shortcut before the user scrolls anywhere), as does `PersonModals.vue` — its root only ever holds closed `<dialog>` elements, which never intersect the viewport, so `client:visible` would never hydrate it and deep links (`#person-modal-<id>`) would break. `NavToggle.vue` and `CookieConsent.vue` use `client:load` since they must be interactive immediately on page load. `panels`, `activities`, and `linkedin` have collections and schemas but no components or pages — nothing rendered them on the old site either; they feed the Python CV tooling only.
- **Tag filtering**: `data/tags.yml` defines the tag set (with per-tag icon/color); `TagFilterBar.vue`/`TagList.vue` are the shared filter-bar and badge components. `FilteredGrid.vue` wraps the tag-filtered grid pattern itself (used by `ProjectsSection`/`NewsSection`): callers fill its `#item` scoped slot with the card and an optional `#modal` scoped slot with that item's modal, rendered once per item (unfiltered) after the grid.
